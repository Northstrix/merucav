...
function getColorAsHsva(
  color: `#${string}` | string | HsvaColor | HslaColor | RgbaColor
): HsvaColor {
  let hsva: HsvaColor;
  if (typeof color === 'string') {
    try {
      if (color.startsWith('rgba')) {
        ...
        hsva = hexToHsva(color as HexColor);
      }
    } catch (e) {
...
export type ColorPickerProps = {
  value?: `#${string}` | string | HsvaColor | HslaColor | RgbaColor;
  type?: 'hsl' | 'rgb' | 'hex' | 'hexa';
  swatches?: HexColor[];
...
export function ColorPicker({
  ...
  const [colorType, setColorType] = React.useState(hideAlpha ? 'hex' : type);
  ...
  const safeAlpha = displayColor.a ?? 1;

  return (
    <div
      className={cn('w-full max-w-[346px] space-y-4 rounded-lg border border-border bg-black p-4', className)}
      style={
        {
    ...
    >
      <Saturation
        hsva={colorHsv}
        onChange={(newColor) => {
          handleValueChange({ ...colorHsv, ...newColor, a: colorHsv.a ?? 1 });
        }}
        style={{
          width: '100%',
          height: 'auto',
      ...
      <div className="space-y-3">
        <Hue
          hue={colorHsv.h}
          onChange={(newHue) => {
            handleValueChange({ ...colorHsv, ...newHue, a: colorHsv.a ?? 1 });
          }}
      ...
      <div className="flex items-center gap-2">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <RadixButton variant="outline" className="shrink-0 justify-between uppercase">
              {colorType}
              <ChevronDownIcon
          ...
          <DropdownMenuContent>
            ...
            >
             ...
             {!hideAlpha && (
                <DropdownMenuCheckboxItem
                    checked={colorType === 'rgba'}
                    onCheckedChange={() => setColorType('rgba')}
                >
                    RGBA
                </DropdownMenuCheckboxItem>
             )}
        ...
        <div className="flex grow">
          ...
          {colorType === 'hex' && (
            <HexColorInput
              color={hsvaToHexa(colorHsv).slice(0, 7)}
              onChange={(newHsva) => handleValueChange({...newHsva, a: 1})}
              onInvalid={setIsInputInvalid}
            />
          )}
          {colorType === 'hexa' && !hideAlpha && (
            <HexColorInput
              color={hsvaToHexa(colorHsv)}
              onChange={handleValueChange}
              onInvalid={setIsInputInvalid}
              alpha
            />
          )}
          {colorType === 'rgba' && !hideAlpha && (
             <ObjectColorInput
              color={hsvaToRgba(colorHsv)}
              onChange={(newColor) => handleValueChange(rgbaToHsva(newColor))}
              onInvalid={setIsInputInvalid}
...
export function ContrastRatioInfo({ color }: ContrastRatioProps) {
  ...
  return (
    <div className="flex items-center gap-4">
      <div 
        className="flex size-10 items-center justify-center rounded-lg bg-grid"
        style={{
...
function HexColorInput({
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const hexRegex = alpha ? /^#([0-9A-Fa-f]{8})$/i : /^#([0-9A-Fa-f]{6})$/i;
  ...
  const targetLength = alpha ? 9 : 7;
  const isInvalid = inputValue.length !== targetLength || (isFocused && !/^#([0-9A-Fa-f]{6,8})$/i.test(inputValue));
...
function ObjectColorInput({
  ...
  const isRtl = useIsRTL();
  const colorType = 'r' in color ? (includeAlpha ? 'rgba' : 'rgb') : 'hsl';
  const keys = colorType === 'rgba' ? ['r', 'g', 'b', 'a'] : colorType === 'rgb' ? ['r', 'g', 'b'] : ['h', 's', 'l'];

  const [inputValues, setInputValues] = React.useState(() =>
    Object.fromEntries(keys.map((key) => {
        let value = (color as any)[key];
        if (value === undefined) {
             if (key === 'a') value = 1;
             else value = 0;
        }
        let stringValue;
        if (key === 'a') stringValue = String(Number(value).toFixed(2));
        else if (key === 's' || key === 'l') stringValue = String(Math.round(value));
        else stringValue = String(Math.round(value));
  ...
  React.useEffect(() => {
    ...
    for (const key of keys) {
      let externalValue;
       const colorVal = (color as any)[key];

      if (key === 'a') {
        externalValue = String(Number(colorVal ?? 1).toFixed(2));
      } else if (key === 's' || key === 'l') {
        externalValue = String(Math.round(colorVal ?? 0));
      } else {
        externalValue = String(Math.round(colorVal ?? 0));
      }
      ...
      if (key !== activeKey && inputValues[key] !== externalValue) {
        ...
        newValues[key] = externalValue;
        if(inputValues[key] !== newValues[key]) changed = true;
      }
    }
    if (changed) {
  ...
  const validate = (key: string, value: number) => {
    ...
    if (key === 'a') return value >= 0 && value <= 1;
    return true;
  };
  ...
  const handleInputChange = (key: string, value: string) => {
    ...
    if (allValid) {
      ...
      if (!('a' in updatedColor) && 'a' in color) {
