...
export function ColorPaletteTools({ onColorAdd, initialColor = '#00A7FA' }: ColorPaletteToolsProps) {
  const { t } = useTranslation();

  // Remove alpha before converting to HSVA for the color picker
  const stripAlpha = (hex: string) => hex.length === 9 ? hex.slice(0, 7) : hex;

  const [baseColor, setBaseColor] = useState<HsvaColor>(hexToHsva(stripAlpha(initialColor)));
  const harmonyTypes = useMemo(() => [
    'analogous', 'triad', 'complementary', 
    'splitComplementary', 'square', 'tetradic', 'convergence'
  ], []);
