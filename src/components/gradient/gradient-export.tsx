...
export function GradientExport({ config, setConfig }: GradientExportProps) {
  ...
  const handleImport = () => {
    try {
      let newConfig = JSON.parse(importJson);

      if (newConfig && typeof newConfig === 'object') {
        // Handle legacy 'orbs' property
        if (newConfig.orbs && !newConfig.shapes) {
            newConfig.shapes = newConfig.orbs;
            delete newConfig.orbs;
        }

        // Basic validation
        if ('shaders' in newConfig) {
            setConfig(newConfig);
